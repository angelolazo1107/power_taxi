...
public class SerialPortMain extends Activity {
    ...
    private void init_view() {
		...
		if ("AT5_V2".equals(Build.MODEL)) 
		{
			final String[] coms = this.getResources().getStringArray(R.array.coms_v2);
			for (int i = 0; i < coms.length; i++) {
				allDevices.add(coms[i]);
			}
		}else
		{
			final String[] coms = this.getResources().getStringArray(R.array.coms_v1);
			for (int i = 0; i < coms.length; i++) {
				allDevices.add(coms[i]);
			}
		}
		ArrayAdapter<String> comsDevices = new ArrayAdapter<String>(this,
				android.R.layout.simple_spinner_item, allDevices);
		comsDevices.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		comSpinner.setAdapter(comsDevices);
            ...
            switch (v.getId()) {
                case R.id.openComButton:
                    if (!isopen) {
                        ...
                        sendEditText
                                .setText("howentest 79 68 6F 5F 74 65 73 74");
                       ...
                       new recDataThread().start();
                        ...
                        if (isAuto == true) {
                            ...
                            openComButton.setText(R.string.text_openComButton);
    ...
    private void open_serialPort() {
        ...
        if ((fd = serialService.serialOpen(comSpinner.getSelectedItem().toString())) < 0) {
            ifopensuccess = false;
            Toast.makeText(SerialPortMain.this, R.string.serialPort_open_fail,
            ...
            if (serialService.serialPortSetting(
                ...
                openComButton.setText(R.string.close_serialport);
