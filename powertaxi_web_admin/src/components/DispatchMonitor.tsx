...
type StatusFilter = 'all' | 'online' | 'onRide' | 'offline';
...
export const DispatchMonitor: React.FC<DispatchMonitorProps> = ({ 
  ...
  const getStatusDetails = (status: string) => {
    switch (status) {
      case 'running':
        return { label: 'ON RIDE', color: 'text-emerald-400 border-emerald-950/40 bg-emerald-950/20', dot: 'bg-emerald-400' };
      case 'idle':
        return { label: 'ONLINE', color: 'text-amber-400 border-amber-950/40 bg-amber-950/20', dot: 'bg-amber-400' };
      case 'disconnected':
        return { label: 'DISCONNECTED', color: 'text-red-400 border-red-950/40 bg-red-950/20', dot: 'bg-red-400' };
      default:
        return { label: 'OFFLINE', color: 'text-textFaint border-borderDark bg-[#1A1E26]', dot: 'bg-textFaint' };
    }
  };
    ...
    <div className="p-8 space-y-6 flex flex-col h-full overflow-y-auto relative">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-panel border border-borderDark rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-textFaint uppercase block tracking-wider">Total Terminals</span>
            <span className="text-2xl font-black text-white">{cntTotal}</span>
          </div>
        </div>
        ...
        <div className="bg-panel border border-borderDark rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-emerald-500/80 uppercase block tracking-wider">On Ride</span>
            <span className="text-2xl font-black text-emerald-400">{cntOnRide}</span>
          </div>
        </div>
        ...
        <div className="bg-panel border border-borderDark rounded-xl p-5 flex items-center gap-4">
          ...
          <div>
            <span className="text-[10px] font-bold text-amber-500/80 uppercase block tracking-wider">Online (Idle)</span>
            <span className="text-2xl font-black text-amber-400">{cntOnline}</span>
          </div>
        </div>

        {/* Card 4: OFFLINE */}
        <div className="bg-panel border border-borderDark rounded-xl p-5 flex items-center gap-4">
          <div className="p-3 bg-red-950/20 text-red-400 rounded-lg">
            <WifiOff size={20} />
          </div>
          <div>
            <span className="text-[10px] font-bold text-red-500/80 uppercase block tracking-wider">Offline</span>
            <span className="text-2xl font-black text-red-400">{cntOffline}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        {/* Tab filters */}
        <div className="flex gap-2 p-1 bg-[#1A1E26] border border-borderDark rounded-xl self-start">
          <button 
          ...
          >
            All <span className="ml-1 text-[10px] opacity-60">({cntTotal})</span>
          </button>
          <button 
            onClick={() => setFilter('onRide')}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
              filter === 'onRide' ? 'bg-emerald-950/40 text-emerald-400 border border-emerald-900/35' : 'text-textFaint hover:text-emerald-400'
            }`}
          >
            On Ride <span className="ml-1 text-[10px] opacity-60">({cntOnRide})</span>
          </button>
          <button 
            onClick={() => setFilter('online')}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
              filter === 'online' ? 'bg-amber-950/40 text-amber-400 border border-amber-900/35' : 'text-textFaint hover:text-amber-400'
            }`}
          >
            Online <span className="ml-1 text-[10px] opacity-60">({cntOnline})</span>
          </button>
          <button 
            onClick={() => setFilter('offline')}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${
              filter === 'offline' ? 'bg-red-950/40 text-red-400 border border-red-900/35' : 'text-textFaint hover:text-red-400'
            }`}
          >
            Offline <span className="ml-1 text-[10px] opacity-60">({cntOffline})</span>
          </button>
        </div>
                ...
                {filteredDevices.map(d => {
                  ...
                  return (
                    <tr key={d.serialNo} className="hover:bg-cardColor/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="text-white font-extrabold text-[13px] tracking-wide font-mono">{d.plateNo}</span>
                      ...
                      {/* Status badge */}
                      <td className="px-6 py-4">
                        <span className={`px-2 py-0.5 border text-[9px] font-extrabold rounded-md ${status.color}`}>
                          {status.label}
                        </span>
                      </td>
                      ...
                      {/* Active Driver */}
                      <td className="px-6 py-4">
                        <span className={`text-xs ${d.currentDriver ? 'text-white' : 'text-textFaint/60 font-normal'}`}>
                          {d.currentDriver || '—'}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex flex-col items-end">
                          <span className="text-white font-mono text-[13px] font-bold">₱{d.dailySales.toLocaleString([], { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                          <span className="text-[8px] text-textFaint flex items-center gap-0.5 uppercase tracking-wider">
                            <TrendingUp size={9} className="text-accentOrange" /> Today
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right font-mono text-white/90">
                        <div className="flex items-center gap-1.5 justify-end">
                          <Clock size={12} className="text-textFaint" />
